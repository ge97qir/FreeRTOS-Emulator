...
TaskHandle_t InvaderNControlTask = NULL;
        ...
        for (int col = 0; col < ENEMY_COLUMNS; col++){
            ...
            if (row == 0 || row == 1){
                setWallProperty(invaders->enemys[row][col].enemy,
                            col * (invaders->enemys[1][1].enemy->w + GAP_SIZE),
                            SCREEN_HEIGHT / 2 - row * (2 * invaders->enemys[row][col].enemy->h), 
                            0, 0, SET_WALL_AXES);
            }else if (row == 2 || row == 3){
                setWallProperty(invaders->enemys[row][col].enemy,
                            ((invaders->enemys[1][1].enemy->w - invaders->enemys[row][col].enemy->w) / 2) 
                            + col * (invaders->enemys[1][1].enemy->w + GAP_SIZE),
                            SCREEN_HEIGHT / 2 - row * (2 * invaders->enemys[row][col].enemy->h), 
                            0, 0, SET_WALL_AXES);
            }else if (row == 4){
        ...
        while(row < ENEMY_ROWS){
            while (col < ENEMY_COLUMNS){
                ...
                if (!invaders->enemys[row][col].dead){
                    ...
                    if (invaders->enemys[row][col].enemy->x2 >= SCREEN_WIDTH - GAP_SIZE / 2){
                        change_direction = 1;
                    }
        ...
        while(row < ENEMY_ROWS){
            while (col < ENEMY_COLUMNS){
                ...
                if (!invaders->enemys[row][col].dead){
                    ...
                    invaders->enemys[row][col].image_state = !invaders->enemys[row][col].image_state;
                    // edge of the screen detection for changing direction
                    if (invaders->enemys[row][col].enemy->x1 <= 0 + GAP_SIZE / 2){
                        change_direction = 1;
                    }
...
unsigned int bulletHitMothership(enemy_t *mothership, bullet_t *bullet_obj){
    if (!mothership->dead){
        if (bullet_obj->bullet->x >= mothership->enemy->x1 + SMALLER_HITBOX && 
            ...
            mothership->dead = DEAD;
            xQueueOverwrite(KilledMothershipQueue, &mothership->dead);
...
void vInvaderNControlTask(void *pvParameters){ 
                ...
                if (xSemaphoreTake(buttons.lock, 0) == pdTRUE) {
                    if (buttons.buttons[KEYCODE(P)]) {
                        ...
                        if (InvaderNControlTask) {
                            vTaskSuspend(InvaderNControlTask);
                        }
                        ...
                        if (xSemaphoreTake(player.lock, portMAX_DELAY) == pdTRUE){
                            if(bulletHitPlayer(&player, &invaders_bullets[b])){
                                ...
                                if(!infinite_lives) {
                                    ...
                                    xQueueOverwrite(KilledPlayerQueue, &player.lives);
                                    bulletPeriodDelay = BULLET_MAX_DELAY;
                                    prevInvaderShootTime = xTaskGetTickCount();
                                    my_bullet.bullet_state = PASSIVE;
                                    xSemaphoreGive(player.lock);
...
int gamesInit(void)
{
    ...
    if (xTaskCreate(vInvaderNControlTask, "InvaderNControlTask",
                    configMAX_PRIORITIES, NULL, mainGENERIC_PRIORITY,
                    &InvaderNControlTask) != pdPASS) {
        PRINT_TASK_ERROR("InvaderNControlTask");
        goto err_invaderncontroltask;
    }
...
err_udpcontrol:
    vTaskDelete(InvaderNControlTask);
err_invaderncontroltask:
